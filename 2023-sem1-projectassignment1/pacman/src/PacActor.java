...
public class PacActor extends GameActor implements GGKeyRepeatListener {
    ...
    private void eatPill(Location location) {
        ...
        if (c.equals(Color.white)) {
            ...
            game.notifyMonsterEatPill();
        } else if (c.equals(Color.yellow)) {
            nbPills++;
            score += 5;
            getBackground().fillCell(location, Color.lightGray);
            game.getGameCallback().pacManEatPillsAndItems(location, "gold");
            game.removeItem(location);
            game.getGameVersion().eatGold(game, location); // notify game
        } else if (c.equals(Color.blue)) {
            getBackground().fillCell(location, Color.lightGray);
            game.getGameCallback().pacManEatPillsAndItems(location, "ice");
            game.removeItem(location);
            game.getGameVersion().eatIce(game, location); // notify game
        }
